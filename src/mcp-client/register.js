...
// 接続済みMCPサーバーのlistTools()結果をNookResonanceのツール定義へ変換しregistry.register()する
async function registerServerTools(server) {
  ...
  try {
    ...
    logger.error(`mcp-client: failed to list tools for "${server.label}", continuing without it`, { error: e.message });
    return 0;
  }
  ...
  logger.info(`mcp-client: registered ${mcpTools.length} tool(s) from "${server.label}"`);
