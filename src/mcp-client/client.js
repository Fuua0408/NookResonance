...
// NookResonanceはCommonJS("type":"commonjs")だが、@modelcontextprotocol/sdkはESM中心のため
...
async function connectServer(serverConfig) {
  ...
  try {
    ...
    const client = new Client({ name: 'nookresonance', version: '1.0.0' }, { capabilities: {} });
    await withTimeout(client.connect(clientTransport), CONNECT_TIMEOUT_MS);

    logger.info(`mcp-client: connected to "${label}" MCP server (${transport === 'http' ? 'http' : 'stdio'})`);
    ...
    logger.error(`mcp-client: failed to connect to "${label}" MCP server, continuing without it`, { reason });
