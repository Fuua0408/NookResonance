...
function parseArgs(argsJson) {
  ...
  try {
    ...
    logger.error('mcp-client: failed to parse args JSON, treating as empty', { error: e.message });
    return [];
  }
}
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (e.code === 'ERR_KEY_MISSING') {
        logger.error(
          `mcp-client: SECRET_ENC_KEY is not set or invalid, skipping "${row.label}" (set SECRET_ENC_KEY in .env to enable it)`
        );
      } else {
        logger.error(`mcp-client: failed to decrypt secrets for "${row.label}", skipping`, { error: e.message });
      }
      continue;
    }
