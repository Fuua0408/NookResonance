...
async function initMcp() {
  ...
  for (const config of configs) {
    if (!config.enabled) {
      logger.info(`mcp-client: "${config.label}" is disabled, skipping`);
      continue;
    }
...
async function closeMcp() {
  for (const server of activeServers) {
    try {
      ...
      logger.info(`mcp-client: closed "${server.label}"`);
    } catch (e) {
      logger.error(`mcp-client: failed to close "${server.label}"`, { error: e.message });
    }
  }
  activeServers = [];
}
