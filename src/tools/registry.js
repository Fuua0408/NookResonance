...
// syncToolsToDb()の孤児無効化は「今回接続に成功したサーバー」基準でしか働かない(一時的な接続断で
// 無効化しないための意図的な設計)。そのため、mcp_servers側で明示的にenabled=0にされたサーバー
// (=接続を試みてすらいない)のtools行は対象外のまま残ってしまう。ここではmcp_servers.enabled=0の
// ラベルについてのみ、対応するtools行を明示的に無効化する。逆(再度enabled=1にした場合)は次回
// 接続成功時にsyncToolsToDbが通常どおりミラーし直す。起動シーケンス(main())とMCP管理APIの
// POST /reload の両方から呼ばれる
function disableToolsForDisabledServers(db) {
  const disabledLabels = db.prepare('SELECT label FROM mcp_servers WHERE enabled = 0').all();
  if (disabledLabels.length === 0) return;

  const disable = db.prepare(
    "UPDATE tools SET enabled = 0, updated_at = datetime('now') WHERE origin = ? AND enabled = 1"
  );
  for (const { label } of disabledLabels) {
    disable.run(`mcp:${label}`);
  }
}
...
module.exports = {
  ...
  syncToolsToDb,
  disableToolsForDisabledServers,
};
